...
#### Restricting the app to just you
This is a single-owner app — set `REACT_APP_OWNER_EMAIL` in `frontend/.env` to your own login email (and `OWNER_EMAIL` near the top of `index.html` if you're using the standalone version) so that no one else can register or sign in, even with the PIN. The "Create Account" option is visible on the sign-in screen, but registration is rejected for any email other than the one you set.
