...
import { Volume2, VolumeX, Zap, Footprints, Download, Cloud, Database, Mail, LogOut, LogIn, Lock } from "lucide-react";
...
function KuohFlowApp({ onLock }) {
  ...
  // 2. Persistent tasks & companion (localStorage)
  const [tasks, setTasks] = useState(() => {
    try {
      const s = localStorage.getItem('kuohflow_tasks_v1');
  ...
  const [selectedCompanion, setSelectedCompanion] = useState(() => {
    try {
      const s = localStorage.getItem('kuohflow_heroine_v1');
  ...
  const [peerageXP, setPeerageXP] = useState(() => {
    try {
      const s = localStorage.getItem('kuohflow_xp_v1');
  ...
  // Optional Firebase cloud sync — activates when firebaseConfig.js is filled in
  useEffect(() => {
    let unsub = () => {};
    if (!user) return () => {};
  ...
  // Persist to localStorage (+ Firestore when Firebase is configured)
  useEffect(() => {
    try { localStorage.setItem('kuohflow_tasks_v1', JSON.stringify(tasks)); } catch (e) {}
    fbSaveDoc({ tasks });
  }, [tasks, user]);

  useEffect(() => {
    try { localStorage.setItem('kuohflow_heroine_v1', selectedCompanion.id); } catch (e) {}
    fbSaveDoc({ heroine: selectedCompanion.id });
  }, [selectedCompanion, user]);

  useEffect(() => {
    try { localStorage.setItem('kuohflow_xp_v1', String(peerageXP)); } catch (e) {}
    fbSaveDoc({ xp: peerageXP });
  }, [peerageXP, user]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center justify-between py-3 gap-3 border-b border-rose-950/40">
            ...
            <div className="flex items-center gap-2.5 flex-wrap relative z-10">
              ...
              {user ? (
                <div className="hc-toolbar hc-status-pill" data-testid="user-pill" title={`Signed in as ${user.email}`}>
                  <Mail className="hc-pill-icon is-on" />
                  <span className="hc-pill-text" style={{ textTransform: 'none' }}>{user.email}</span>
                  <button onClick={() => { audio.playClick(); logout(); }} data-testid="logout-button" title="Sign out" className="ml-1 p-1 rounded-lg hover:bg-rose-950/60 text-slate-400 hover:text-rose-300 transition-colors">
