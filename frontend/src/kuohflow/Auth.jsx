...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    initFirebase().then(async (configured) => {
      ...
      unsubscribe = auth.onAuthStateChanged((firebaseUser) => {
        setUser(firebaseUser && sessionStorage.getItem(PIN_SESSION_KEY) === "1" ? {
          id: firebaseUser.uid,
  ...
  const register = async (email, password, name, pin) => {
    ...
    try {
      ...
      setUser({
        id: result.user.uid,
        email: result.user.email || "",
        name: name.trim() || result.user.email?.split("@")[0] || "",
      });
    } catch (error) {
      sessionStorage.removeItem(PIN_SESSION_KEY);
      throw error;
  ...
  return <AuthContext.Provider value={{ user, login, register, logout, authError }}>{children}</AuthContext.Provider>;
}
