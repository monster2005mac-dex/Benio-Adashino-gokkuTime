...
export const fbGetDoc = async (field) => {
  ...
  try {
    const snap = await db.collection("kuohflow_users").doc(firebase.auth().currentUser.uid).get();
...
export const fbSaveDoc = (fields) => {
  if (!ready || !firebase.auth().currentUser) return;
  db.collection("kuohflow_users").doc(firebase.auth().currentUser.uid).set(fields, { merge: true }).catch(
    (e) => console.warn("Firebase write failed:", e.message)
  );
};

export const fbListenDoc = (callback) => {
  if (!ready || !firebase.auth().currentUser) return () => {};
  return db.collection("kuohflow_users").doc(firebase.auth().currentUser.uid).onSnapshot(
