...
export default function GirlfriendChat({ audio, stats, user }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch(`${API}/chat/history?session_id=${encodeURIComponent(getSessionId(user))}`, { headers: { Authorization: `Bearer ${await getToken()}` } });
  ...
  const clearChat = async () => {
    ...
    try { await fetch(`${API}/chat/history`, { method: "DELETE", headers: { Authorization: `Bearer ${await getToken()}` } }); } catch (e) {}
  ...
  const handleImagePick = async (e) => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch(`${API}/upload`, { method: "POST", headers: { Authorization: `Bearer ${await getToken()}` }, body: fd });
